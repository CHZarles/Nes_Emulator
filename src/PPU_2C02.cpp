...
uint8_t PPU_2C02::cpuRead(uint16_t addr, bool rdonly)
{
	uint8_t data = 0x00;

	if (rdonly) // 这种情况单纯是为了调试
	{
		// 仅读取ppu寄存器的内容(不改变值)
		switch (addr)
		{
        ...
        for (uint8_t i = 0; i < sprite_count; i++)
        {
            ...
            if (!control.sprite_size)
            {
                ...
                else
                {
                    ...
                    sprite_pattern_addr_lo = (control.pattern_sprite << 12) // Which Pattern Table? 0KB or 4KB offset
                        |
                        (spriteScanline[i].id << 4) // Which Cell? Tile ID * 16 (16 bytes per tile)
                        // 翻转精灵
